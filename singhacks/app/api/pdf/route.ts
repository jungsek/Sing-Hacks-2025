...
export async function POST(request: Request) {
  ...
  if (documents.length === 0 && body?.docId) {
    try {
      ...
      // eslint-disable-next-line no-console
      console.warn(
        "Could not fetch related documents for PDF generation:",
  ...
  const assessments =
    body?.assessments && Array.isArray(body.assessments)
      ? body.assessments
      : documents.map((doc, idx) => ({
          id: doc.id,
          riskLevel: idx % 3 === 0 ? "High" : idx % 3 === 1 ? "Medium" : "Low",
          summary:
            "Mock summary: detailed assessment will be provided by the AI agent when integrated.",
        }));

  const docProps = {
    filename,
    generatedOn:
      body.generatedOn || new Date().toLocaleString("en-GB", { timeZone: "Asia/Singapore" }),
    ...
    overallSummary:
      body?.overallSummary ||
      "Mock overall summary: a summary of combined document risk. AI agent integration will enhance this.",
    assessments,
  };
}
